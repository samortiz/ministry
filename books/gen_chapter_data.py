...
for book in currentBooks:
  # Determine if we need to look up chapters for this book, or if it's already done
  if ( os.path.exists(os.path.join(chapDir, book, "toc_with_links.html"))  # Data exists already 
       or len(os.listdir(os.path.join(chapDir, book))) == 1                # There is only one chapter  
     ):  
    continue
  ...
  if os.path.exists(tocFile):
    ...
    if (os.path.exists(os.path.join(cleanDir, book, "page_001.html"))):  
      ...
      #If no chapters were found in the toc then what's up with having a TOC at all?
      if len(chapters) == 0:
        print "ERROR!  No chapters found when parsing the first page as a TOC, but there is a chapter 2!"
        sys.exit # Error, this will exit :) 
      else:
        # We found a toc in the first page, so copy that to the  chapter_Table of Contents.html file
         writeStringToFile(tocFile, tocHtml)
    else:
      print "ERROR! We have chapter data, but no clean data to lookup pages for."
      sys.exit
  ...
  for chapter in chapters:
    ...
    # Enclose the table of contents chapter text with a link to the chapter page
    if (tocHtml.find(chapTitle) > 0):
      tocHtml = tocHtml.replace(chapTitle, "<a href='page_"+chapPageNum+".html'>"+chapTitle+"</a>", 1)
    else:
      print "Error!  The chapter title "+chapTitle+" is not in toc!"
      sys.err
  
  # Write out the newly generated table of contents (with all the links added)
  if tocHtml != "": 
