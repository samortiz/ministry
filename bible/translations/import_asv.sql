...
create index import_verse_idx on import(verse_id);


--- ---------- Validation ------------------
-- Verses that don't exist in the new version
select * from import where verse_id is null;

-- Verses that are in RcV that aren't in the new version (Rev 12:18, all the titles of the Psalms)
select * from verse where id not in (select distinct verse_id from import);


--- ----------- Put the data live -------------
alter table verse add column asv text;
update verse set asv=(select asv from import where verse_id=verse.id);
