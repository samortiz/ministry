...
chapter 7 of John talks about living water and verse 39 mentions the Spirit
In verses 10 and 12 Paul concludes,
vv. 16 and 17
John 1:1,4 and 14, 17-18 and 28 to 29 but not,99 (no verse immediately before it)
vv. 1-2,4-5 and 7
John 3:14,12:24
vv. 3-5
In verses 15 to 16 we see
JOhn 3:15 through 16
something seen in vv. 6 to 7 it is shown
we see in v 9 (no period, no match!)
1Sam 2 in verse 3
3 John 5
Jude 2-6
John 1:12-13
...
*ERROR* In the third chapter of Romans, verse 23 says that all have sinned
"""
# #########################
...
verse_full = "("+chapter+" *(:|[vV]erse) *([0-9]{1,3})( *(to|-|through) *(([0-9]{1,3}) *: *){0,1}([0-9]{1,3})){0,1})"
verse_no_chapters = "("+book_with_no_chapters+" *([vV]erse){0,1} *([0-9]{1,3})( *(to|-|through) *(([0-9]{1,3}) *: *){0,1}([0-9]{1,3})){0,1})"
verse_vv = "(vv\. *([0-9]{1,3}) *(to|-|through) *([0-9]{1,3}))"
verse_v = "(v\. *([0-9]{0,3}))"
verse_verses = "([vV]erses *([0-9]{1,3})( *(to|-|through) *([0-9]{1,3})){0,1})"
verse_verse = "([vV]erse *([0-9]{1,3}))"
verse_just_numbers = "(([0-9]{1,3}) *: *([0-9]{1,3})( *(to|-|through) *(([0-9]{1,3}) *: *){0,1}([0-9]{1,3})){0,1})"

# Multiple Verses (separated by commas) 
# This requires special handling (seperating the verses into chunks)
verse_multiple = "( *(,|and) *([0-9]{1,3})( *(to|-|through) *([0-9]{1,3})){0,1}){1,}"


# Store all the regex and their associated variables
patterns = []

# These numbers come from the parameters to the storeVerseData function
#             bookGrp, chapGrp, bookChapGrp, verseGrp, endChapGrp, endVerseGrp

# Verse Patterns
patterns.append([verse_full, -1, -1, 2, 27, 31, 32])
patterns.append([verse_no_chapters, 1, -1, -1, 6, -1, 11])
patterns.append([verse_vv, -1, -1, -1, 2, -1, 4])
patterns.append([verse_v,  -1, -1, -1, 2, -1, -1])
patterns.append([verse_verses,-1, -1, -1, 2, -1, 5])
patterns.append([verse_verse, -1, -1, -1, 2, -1, -1])
patterns.append([verse_just_numbers, -1, 2, -1, 3, 7, 8])
patterns.append([verse_just_numbers, -1, 2, -1, 3, 7, 8])

#This one is handled seperately, so it's not put into the patterns array (it needs to be split up)
verse_multiple_pattern_array = [verse_multiple, -1, -1, -1, 3, -1, 6]

#Chapter Patterns
patterns.append([chapter_full, 2, 11, -1, -1, -1, -1])
patterns.append([chapter_verbose, 3, 2, -1, -1, 1, -1])
patterns.append([chapter_brief, -1, 2, -1, -1, 1, -1])

#Book Pattern
patterns.append([book, 1, -1, -1, -1, 1, -1])
...
if text != "" : 
  ...
  for patData in patterns : 
    pattern = patData[0]
   
    #Go through each match (reference or reference fragment) found
    for match in re.finditer(pattern, text):
      matchStr =  match.group(0)
      matchPos = match.start()
      endPos = matchPos + len(matchStr)
      matches.append([matchPos, endPos, matchStr,patData])

  # Uniquify the matches (remove overlapping matches)
  matches.sort() # Order by startPos
  # Remove overlapping items
  for matchData in matches:
    start = matchData[0]
    end = matchData[1]
    matchStr = matchData[2]
    matchSize = len(matchStr)
    #print str(start)+" - "+str(end)+" : "+matchStr+"<br>"
    
    umIndex = 0
    foundMatch = 0
    for uniqueMatch in uniqueMatches:
      startUnique = uniqueMatch[0]
      endUnique = uniqueMatch[1]
      if not ((start > endUnique) or (end < startUnique)): #There is any overlap
        if (matchSize > len(uniqueMatch[2])):
          uniqueMatches[umIndex] = matchData
        foundMatch = 1
        break
      umIndex += 1
    if foundMatch == 0:
      uniqueMatches.append(matchData)


  #Check for syntax doing multiple verses using , or 'and' syntax
  for match in re.finditer(verse_multiple, text):
    matchStr =  match.group(0)
    start = match.start()
    end = start + len(matchStr)
    doInsert = 0
    for uMatch in uniqueMatches:
      uStart = uMatch[0]
      uEnd = uMatch[1]
      uStr = uMatch[2]
      if (start == uEnd):
        doInsert = 1
      if ((start <= uStart) and (end > uStart) and (end <= uEnd)): 
        doInsert = 0 #Canceled by overlap with another pattern (the existing patterns take precendence)
    if doInsert == 1: 
      #Split the group up by "," and "and" 
      matchStrArr = re.split(",|and", matchStr)
      firstLoop = 1
      for newStr in matchStrArr:
        # The first array always has nothing or whitespace, because the pattern starts with a seperator
        if (firstLoop == 1): 
          firstLoop = 0
          continue
        # Calculate the position in original string (this may fail on duplicate references, which may cause the ordering to be incorrect)
        newStart = start + matchStr.find(newStr)
        newEnd = newStart + len(newStr) + 1
        matchData = [newStart, newEnd, ","+newStr, verse_multiple_pattern_array]
        uniqueMatches.append(matchData)
        # print "Adding "; print matchData; print "<br>"
  

  # Re-order the unique matches, because the above loop messed up the order by adding items to the end (for context the order is important)
  uniqueMatches.sort()
    


  # Go through all the unique matches and look up the verses for them
  for match in uniqueMatches:
    matchStart = match[0]
    matchStr = match[2]
    patData = match[3]
    #storeVerseData(ref,    pattern   ,bookGrp   ,chapGrp   ,bookChapGrp,verseGrp  ,endChapGrp,endVerseGrp,strPos):
    storeVerseData(matchStr,patData[0],patData[1],patData[2],patData[3] ,patData[4],patData[5],patData[6] ,matchStart)


  # Display all the verses found 
